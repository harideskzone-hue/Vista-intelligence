...
class GenericTemporalVoter:
    ...
    def decide(self) -> dict:
        ...
        if obs_count >= self.min_observations and avg_evidence >= self.min_evidence:
            status = "CONFIRMED"
...
def run(video_input):
        ...
        if results and results[0].boxes and results[0].boxes.id is not None:
            ...
            for box, track_id, conf, cls_id in zip(boxes, track_ids, confidences, class_ids):
                ...
                if track_id not in track_history:
                    track_history[track_id] = {"first_seen": frame_idx, "class": cls_name}
                    ...
                    if plate_crop.size > 0:
                        ...
                        try:
                            ...
                            if preds:
                                ...
                                # Quality score heuristic (box height relative to frame height + detection confidence)
                                lp_height = gy2 - gy1
    ...
    summary_log = {
        ...
        "plate_detections": total_plate_detections,
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python scripts/run_video_validator.py <path_to_video.mp4>")
    else:
        run(sys.argv[1])
